...
let view (router: Router<_, _, _>) model dispatch =
    concat {
        ...
        div {
            ...
            match model.Compositions with
            | Deferred.Loading -> loadingView
            | Deferred.LoadFailed e ->
                errorNotificationWithRetry "Fehler beim Laden." (fun () -> dispatch LoadActiveCompositions)
            | Deferred.Loaded loadedCompositions ->
                match loadedCompositions.SelectedComposition with
                ...
                | Some (_composition, Deferred.LoadFailed _e) ->
                    errorNotificationWithRetry "Fehler beim Laden." (fun () -> dispatch LoadVoices)
